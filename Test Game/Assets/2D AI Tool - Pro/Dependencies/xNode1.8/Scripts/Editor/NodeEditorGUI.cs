...
namespace XNodeEditor {
    ...
    public partial class NodeEditorWindow {
                ...
                case NoodlePath.AITool:
                    for (int i = 0; i < length - 1; i++)
                    {
                        ...
                        if (gridPoints[i].x <= gridPoints[i + 1].x - (50 / zoom))
                        {
                            ...
                            if (Mathf.RoundToInt(start_1.y) + 1 != Mathf.RoundToInt(end_1.y))
                            {
                                start_1.x = midpoint;
                                end_1.x = midpoint;
                            }
                            ...
                            {
                                DrawAAPolyLineNonAlloc(thickness, gridPoints[i], start_1);
                                DrawArrows(gridPoints[i], start_1, gradient.Evaluate(1));

                                Color color = Handles.color = gradient.Evaluate(0.5f);
                                DrawAAPolyLineNonAlloc(thickness, start_1, end_1);
                                DrawArrows(start_1, end_1, color);

                                color = Handles.color = gradient.Evaluate(1f);
                                DrawAAPolyLineNonAlloc(thickness, end_1, gridPoints[i + 1]);
                                DrawArrows(end_1, gridPoints[i + 1], color);
                            }
                            else
                            {
                                DrawAAPolyLineNonAlloc(thickness, gridPoints[i], start_1);
                                ...
                                DrawAAPolyLineNonAlloc(thickness, start_1, end_1);
                                DrawArrows(start_1, end_1, gradient.Evaluate(1));

                                DrawAAPolyLineNonAlloc(thickness, end_1, gridPoints[i + 1]);
                                DrawArrows(end_1, gridPoints[i + 1], gradient.Evaluate(1));
                            }
                        }
                        else
                        {
                            ...
                            if (i == length - 2)
                            {
                                ...
                                if ((start_1.x - gridPoints[i + 1].x) / 10 > 25 / zoom)
                                {
                                    DrawAAPolyLineNonAlloc(thickness, start_1, start_2);
                                    DrawArrows(start_1, start_2, color);
                                ...
                                DrawAAPolyLineNonAlloc(thickness, start_2, start_1);
                                DrawArrows(start_2, start_1, color);

                                DrawAAPolyLineNonAlloc(thickness, start_1, end_2);
                                DrawArrows(start_1, end_2, color);

                                color = Handles.color = gradient.Evaluate(0.75f);
                                DrawAAPolyLineNonAlloc(thickness, end_2, end_1);
                                DrawArrows(end_2, end_1, color);
                            ...
                            {
                                start_2.y = gridPoints[i + 1].y;
                                end_2 = gridPoints[i + 1];

                                Color color = Handles.color = gradient.Evaluate(1f);

                                DrawAAPolyLineNonAlloc(thickness, gridPoints[i], start_1);
                                DrawArrows(gridPoints[i], start_1, color);

                                DrawAAPolyLineNonAlloc(thickness, start_1, start_2);
                                DrawArrows(start_1, start_2, color);

                                DrawAAPolyLineNonAlloc(thickness, start_2, end_2);
                                DrawArrows(start_2, end_2, color);
                            }
                        }
                    }
                    break;
        ...
        void DrawArrows(Vector2 point_a, Vector2 point_b, Color color)
        {
            GUI.color = color;

            if (point_a.x == point_b.x && Mathf.Abs(point_b.y - point_a.y) < 1 || point_a == point_b)
                return;

            float distance = Vector2.Distance(point_a, point_b);
            float perDist = 100;
            int total = 1;

            if (distance > perDist)
                total = (int)(distance / perDist);

            Vector2 size = new Vector2() { x = 15 / zoom, y = 15 / zoom };

            Vector2 vectorBetween = point_b - point_a;

            float rotationAngle = Mathf.Atan2(vectorBetween.x, vectorBetween.y) * Mathf.Rad2Deg;

            for (int arrowCount = 1; arrowCount <= total; arrowCount++)
            {
                Vector2 distanceToAdd = (point_b - point_a) / (total + 1);
                Vector2 arrowPosition = point_a + distanceToAdd * arrowCount;

                Rect rect = new Rect(arrowPosition, size);
                rect.center = arrowPosition;

                switch (Mathf.RoundToInt(rotationAngle))
                {
                    case 90:
                        GUI.DrawTexture(rect, NodeEditorResources.noodleTexture_RIGHT);
                        break;
                    case -90:
                        GUI.DrawTexture(rect, NodeEditorResources.noodleTexture_LEFT);
                        break;
                    case 180:
                        GUI.DrawTexture(rect, NodeEditorResources.noodleTexture_UP);
                        break;
                    case 0:
                        GUI.DrawTexture(rect, NodeEditorResources.noodleTexture_DOWN);
                        break;
                }
